...
exports.update = async (req, res) => {
  try {
    ...
    return res.status(200).json({ status: true, message: "finally, FAQ has been updated by the admin.", FaQ: FaQ });
  } catch (error) {
    console.log(error);
    return res.status(500).json({
      status: false,
...
exports.destroy = async (req, res) => {
  try {
    ...
    return res.status(200).json({ status: true, message: "finally, FAQ has been deleted by the admin." });
  } catch (error) {
    return res.status(500).json({
      status: false,
      error: error.message || "Internal Server error",
