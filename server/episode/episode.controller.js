...
exports.update = async (req, res) => {
  try {
    ...
    if (req.body.image) {
      ...
      episode.updateType = Number(req.body.updateType); //always be 1
      episode.convertUpdateType.image = Number(req.body.convertUpdateType.image); //always be 1
    ...
    if (req.body.videoUrl) {
      ...
      episode.updateType = Number(req.body.updateType); //always be 1
      episode.convertUpdateType.videoUrl = Number(req.body.convertUpdateType.videoUrl); //always be 1
...
exports.get = async (req, res) => {
  try {
    ...
    return res.status(200).json({ status: true, message: "finally, get all episodes by the admin.", episode });
  } catch (error) {
    return res.status(500).json({
      status: false,
      error: error.message || "Internal Server Error",
...
exports.destroy = async (req, res) => {
  try {
    ...
    return res.status(200).json({ status: true, message: "finally, episode deleted by the admin." });
  } catch (error) {
    console.log(error);
