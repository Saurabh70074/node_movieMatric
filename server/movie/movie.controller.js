...
//get details IMDB id or title wise for admin
...
exports.update = async (req, res) => {
  try {
    ...
    console.log("query in update movie or webSeries:", req.query);
...
exports.getAll = async (req, res) => {
  try {
    ...
    if (req.query.search) {
      const totalMoviesWebSeries = await Movie.find({ ...matchQuery, ...searchQuery }).countDocuments();
      const movie = await Movie.find({ ...matchQuery, ...searchQuery })
        .populate(query)
        .sort({ createdAt: -1 })
        .skip((start - 1) * limit)
        .limit(limit);
      ...
      const totalMoviesWebSeries = await Movie.find(matchQuery).countDocuments();
      const movie = await Movie.find(matchQuery)
        .populate(query)
        .sort({ createdAt: -1 })
        .skip((start - 1) * limit)
