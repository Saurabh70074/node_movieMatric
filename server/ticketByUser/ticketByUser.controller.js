...
exports.ticketRaisedByUser = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({ status: true, message: "finally, Ticket has been raised by the user.", ticketByUser: ticketByUser });
  } catch (error) {
    console.log(error);
  ...
  try {
    ...
    if (req.query.status === "Pending") {
      ...
      return res.status(200).json({
        status: true,
        message: "finally, get all raised tickets for admin.",
...
exports.ticketSolve = async (req, res) => {
  try {
    ...
    return res.status(200).json({ status: true, message: "finally, ticket of the particular user has been solved.", raisedTicket });
  } catch (error) {
    console.log(error);
